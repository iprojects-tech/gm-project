...
OLLAMA_API_URL = "http://localhost:11434/api/generate"  # Por defecto, Ollama corre en este puerto
DOCS_DIR = ""  # Se define dinámicamente por el usuario

model = SentenceTransformer("paraphrase-MiniLM-L6-v2")  # O el que sea
print("Dimensión de embeddings:", model.get_sentence_embedding_dimension())  # Te dice 384 o 512
...
def analyze_directory(req: AnalyzeRequest):
    ...
    DOCS_DIR = req.path
    files = glob.glob(os.path.join(DOCS_DIR, '**', '*.pdf'), recursive=True) + glob.glob(os.path.join(DOCS_DIR, '**', '*.docx'), recursive=True)
    processing_status.update({"progress": 0, "total": len(files), "done": False, "message": "Processing..."})
        ...
        for i, f in enumerate(files):
            ...
            if hasattr(embeddings, 'shape'):
                if embeddings.shape[1] != vs.index.d:
                    print(f"ERROR: El embedding shape {embeddings.shape[1]} no coincide con FAISS ({vs.index.d}) en archivo {f}")
                    processing_status['progress'] = int(100*(i+1)/len(files))
                    continue
                if len(embeddings) != len(chunks):
                    print(f"SKIP: embeddings ({len(embeddings)}) y chunks ({len(chunks)}) no coinciden en {f}")
                    processing_status['progress'] = int(100*(i+1)/len(files))
                    continue
            else:
...
def call_ollama_mistral(context: str, question: str, lang: str = "en") -> str:
    """
    Llama a Ollama/Mistral usando la API local.
    """
    prompt = f"""
...
def chat(query: ChatQuery):
    ...
    for c in results:
        ...
        if c.get('is_image', False):
            ...
            images.append(f"/images/{os.path.basename(c['image_path'])}")
        else:
    ...
    return {"answer": answer, "images": images, "sources": list(set(sources))}
